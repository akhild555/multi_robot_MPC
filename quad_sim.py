...
def closest_cop(cop_positions, robber_position):
    ...
    close_cop = np.array([furthest_point[0], furthest_point[1], 0,0,0,0])
...
def robber_desired_pos(cop_center, extents):
    ...
    for i in extents:
        ...
        if dist_new > dist and dist_new < 1:
            print(dist_new)
            dist = dist_new
            furthest_point = i
...
def simulate_quadrotor(x0_cops, x0_robber, quad_cops, quad_robber, tf, num_cops = 2):
    ...
    while eps_check and t[-1] < tf:
        ...
        x_rob_des = robber_desired_pos(close_cop, env_extents)
        ...
        # current_time = t[-1]
        x_cop_des = np.array([x_robber[-1][0],x_robber[-1][1], 0,0,0,0])
